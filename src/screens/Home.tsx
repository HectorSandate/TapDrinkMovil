...
import { View, Text, Image, TextInput, TouchableOpacity, Alert, ActivityIndicator } from 'react-native'; // Importa ActivityIndicator desde react-native
  ...
  const handleSearch = async () => {
    try {
      if (!nombreCocktail && !filtro) {
        Alert.alert('Advertencia', 'Debes llenar por lo menos algún campo para realizar la búsqueda.');
        return;
      }
      ...
      if (response.data && response.data.length > 0) {
        ...
        Alert.alert('Información', 'No se encontraron recetas con esas características.');
      }
    } catch (error) {
      if (error.response && error.response.status === 404) {
        Alert.alert('Error', 'No se encontraron recetas con esas características.');
      } else {
    ...
    <LinearGradient colors={['#141517', '#FFC107']} locations={[0.5, 8]} style={styles.container}>
      ...
      <View style={styles.filterContainer}>
        ...
        <TouchableOpacity onPress={handleClearFilter} style={styles.filterButton}>
          <Image source={require('../../assets/sin-filtro.png')} style={styles.filterIcon} />
        </TouchableOpacity>
      </View>
      <View style={styles.buttonContainer}>
        <TouchableOpacity onPress={handleSearch} style={[styles.button, loading && styles.disabledButton]} disabled={loading}>
          {loading ? (
